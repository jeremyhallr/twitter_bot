...
def tweet_something(tweet)
  login_twitter.update(tweet)
end

# Get the n latest tweets containing content and favorite them

def like_tweet_with_content(content, n=1)
  login_twitter.search(content, result_type: "recent").take(n).each do |tweet|
    login_twitter.favorite(tweet)
    puts tweet.user.screen_name + tweet.text # tweet.user.screen_name
  end
end

# Send some support tweets to n journalists followed by #bonjour_monde

def tweet_support_journalist(n)
  n.times {
    login_twitter.update("#{select_random_journalist(journalist_list)} #{select_random_encouragement(encouragement_list)} #bonjour_monde")
  }
end

# Get n latest tweets with bonjour_monde and follow their authors

def follow_latest_bonjour_monde(n)
  array_recent_helloworld = []
  login_twitter.search("#bonjour_monde", result_type: "recent").take(n).each do |tweet|
    array_recent_helloworld.push(tweet.user.id)
  end
  array_recent_helloworld.uniq.each {|twitter_id|
